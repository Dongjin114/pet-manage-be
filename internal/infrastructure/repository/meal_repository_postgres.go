...
// GetAll 모든 급식 아이템 조회
func (r *MealRepository) GetAll(ctx context.Context) ([]entities.MealItem, error) {
	query := `
	...
	var items []entities.MealItem
	for rows.Next() {
		var item entities.MealItem
...
func (r *MealRepository) GetByID(ctx context.Context, id uint) (*entities.MealItem, error) {
	query := `
		SELECT id, pet_user_role_id, data_type, meal_type, meal_category, name, unit_type,
		       created_at, modified_at, deleted_at, is_deleted
		FROM meal_items 
...
// Create 급식 아이템 생성
func (r *MealRepository) Create(ctx context.Context, item *entities.MealItem) error {
	query := `
...
// Update 급식 아이템 수정
func (r *MealRepository) Update(ctx context.Context, id uint, item *entities.MealItem) error {
	query := `
		UPDATE meal_items 
...
func (r *MealRepository) GetByType(ctx context.Context, mealType entities.MealType) ([]entities.MealItem, error) {
	query := `
		SELECT id, pet_user_role_id, data_type, meal_type, meal_category, name, unit_type,
		       created_at, modified_at, deleted_at, is_deleted
		FROM meal_items 
